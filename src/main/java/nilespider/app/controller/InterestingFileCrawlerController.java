...
import nilespider.app.model.InterestingFileCrawler;
import nilespider.app.views.components.interfaces.AtomicComponents;

import java.util.HashSet;

public class InterestingFileCrawlerController implements AtomicComponents {
    private Thread thread;
    private HashSet<String> visitedUrls;
    private String url, query;
    public InterestingFileCrawlerController(String url){
        visitedUrls = new HashSet<>();
        thread = new Thread(new InterestingFileCrawler(url, visitedUrls));
        this.url = url;
        this.query = query;
        thread.start();
    }
